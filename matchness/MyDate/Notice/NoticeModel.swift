...
/*
 プロトコル判定用
 */
enum NoticeModelDelegateError : Error {
    case start;
    case complete;
    case failed;
}
...
class NoticeModel: NSObject {
    //プロトコル
    var delegate: NoticeModelDelegate?;
    ...
    var array1: [String] = []
    var array2: Dictionary<String, ApiNoticeList> = [:]
...
extension NoticeModel : ApiRequestDelegate {
    ...
    public func onParse(_ json: JSON){
        ...
        json.forEach { (key, json) in
            //データを変換
            let data: ApiNoticeList? = ApiNoticeList(json: json);
            if (page != 1) {
